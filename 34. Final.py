...
# Set page configuration at the start of the script
st.set_page_config(page_title="Circular Updater", page_icon="📄")
...
def update_circulars():
    ...
    total_downloads = 0
    total_existing = 0
    for page in range(6):  # This will check pages 0 to 5
        url = f"{base_url}?page={page}"
        st.write(f"Checking page {page}...")
        ...
        try:
            ...
            table = soup.find('table', class_='views-table')  # Adjust the class if needed
            if table:
                rows = table.find_all('tr')
                for row in rows[1:]:  # Skip the header row
                    columns = row.find_all('td')
                    if len(columns) >= 3:  # Ensure we have enough columns
                        title = columns[1].text.strip()  # The title is in the second column
                        download_link = columns[2].find('a', href=True)  # The download link is in the third column
                        ...
                        if download_link and title:
                            ...
                            # Remove any invalid characters from the title for the filename
                            safe_title = re.sub(r'[^\w\-_\. ]', '_', title)
                            file_name = os.path.join(circulars_folder, f"{safe_title}.pdf")
                            
                            if not os.path.exists(file_name):
                                st.write(f"Downloading: {title}")
                                
                                try:
                                    with requests.get(download_url, timeout=10, stream=True) as file_response:
                                    ...
                                    st.error(f"An error occurred while downloading {title}: {str(file_err)}")
                            else:
                                st.info(f"Already exists: {title}")
                                total_existing += 1
            else:
                st.info(f"No table found on page {page}.")
       
        except requests.RequestException as e:
            st.error(f"An error occurred while fetching circulars from page {page}: {str(e)}")
            continue
