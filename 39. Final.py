...
def update_circulars():
    ...
    for page in range(6):  # This will check pages 0 to 5
        ...
        try:
            ...
            download_links = soup.find_all('a', href=True, text=lambda text: text and text.strip().lower().startswith('download'))
            ...
            if download_links:
                for link in download_links:
                    ...
                    file_name = os.path.join(circulars_folder, f"{title}.pdf")
                    
                    if not os.path.exists(file_name):
                        download_circular(download_url, file_name, title)
                        total_downloads += 1
                    else:
                        st.info(f"Already exists: {title}")
                        total_existing += 1
            
            # Method 2: Check for table structure
            else:
                ...
                if table:
                        ...
                        if len(cells) >= 3:
                            ...
                            if download_link:
                                ...
                                if file_id not in downloaded_files:
                                    ...
                                    if not os.path.exists(file_name):
                                        if download_circular(download_url, file_name, title):
                                            total_downloads += 1
                    ...
                    st.warning(f"No table or download links found on page {page}.")
...
def download_circular(download_url, file_name, title):
    try:
