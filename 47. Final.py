...
def display_user_manual():
    ...
    # Add download button for the user manual PDF
    with open("UserManual_Demo.pdf", "rb") as pdf_file:
        base64_pdf = base64.b64encode(pdf_file.read()).decode('utf-8')
    
    pdf_display = f'<a href="data:application/pdf;base64,{base64_pdf}" download="UserManual_Demo.pdf">Download User Manual</a>'
...
def update_circulars():
    ...
    existing_files = set(os.listdir(circulars_folder))
    
    all_circulars = []
    
    for i in range(6):
        url = f"{base_url}?page={i}"
        st.write(f"Checking page {i}...")
                ...
                for row in rows:
                    ...
                    if len(cells) >= 3:
                        ...
                        if download_link:
                            ...
                            all_circulars.append((title, download_url, file_size))
            else:
                continue
       
        except requests.RequestException as e:
            st.error(f"An error occurred while fetching circulars from page {i}: {str(e)}")
            continue
    
    st.write(f"Total circulars found: {len(all_circulars)}")
    
    for title, download_url, file_size in all_circulars:
        try:
            ...
            if content_disposition:
                ...
                parsed_url = urlparse(file_response.url)
                filename = unquote(os.path.basename(parsed_url.path))
            
            if not filename.lower().endswith('.pdf'):
                filename += '.pdf'
            
            file_path = os.path.join(circulars_folder, filename)
    ...
    st.write(f"Total processed: {total_downloads + total_existing}")

    if total_downloads + total_existing < len(all_circulars):
        st.warning(f"Note: {len(all_circulars) - (total_downloads + total_existing)} circulars were not processed. This might be due to download errors.")


# New function to extract circular metadata
def extract_circular_metadata(circular_folder, metadata_folder, titles_folder):
    circular_files = os.listdir(circular_folder)
    metadata_files = os.listdir(metadata_folder)
    titles_files = os.listdir(titles_folder)
    circulars = []

    for circular_file in circular_files:
        circular_name, _ = os.path.splitext(circular_file)
        metadata_file = f"{circular_name}.txt"
        titles_file = f"{circular_name}.txt"
        
        if metadata_file in metadata_files and titles_file in titles_files:
            metadata_path = os.path.join(metadata_folder, metadata_file)
            titles_path = os.path.join(titles_folder, titles_file)
            try:
                with open(metadata_path, 'r', encoding='utf-8') as f:
                    content = f.read()
                    file_no_match = re.search(r'File No\.:\s*(.*)', content)
                    issuer_match = re.search(r'Issuer:\s*(.*)', content)
                    date_match = re.search(r'Date:\s*(.*)', content)
                    signatory_match = re.search(r'Signatory:\s*(.*)', content)

                with open(titles_path, 'r', encoding='utf-8') as f:
                    title_content = f.read()
                    title_match = re.search(r'Title:\s*(.*)', title_content)

                if file_no_match and issuer_match and date_match and signatory_match and title_match:
                    circular_info = {
                        'file_no': file_no_match.group(1).strip(),
                        'issuer': issuer_match.group(1).strip(),
                        'date': date_match.group(1).strip(),
                        'signatory': signatory_match.group(1).strip(),
                        'title': title_match.group(1).strip(),
                        'pdf': circular_file
                    }
                    circulars.append(circular_info)
                else:
                    st.warning(f"Failed to extract metadata or title from {metadata_file} or {titles_file}")

            except Exception as e:
                st.warning(f"Error processing {metadata_file} or {titles_file}: {str(e)}")

    return circulars


# New function to display circulars table of contents
def display_circulars_table_of_contents(circulars, sort_option):
    if sort_option == 'File No.':
        sorted_circulars = sorted(circulars, key=lambda x: x['file_no'])
    elif sort_option == 'Issuer':
        sorted_circulars = sorted(circulars, key=lambda x: x['issuer'])
    elif sort_option == 'Date':
        sorted_circulars = sorted(circulars, key=lambda x: datetime.strptime(x['date'], '%d-%m-%Y'))
    elif sort_option == 'Signatory':
        sorted_circulars = sorted(circulars, key=lambda x: x['signatory'])
    elif sort_option == 'Title':
        sorted_circulars = sorted(circulars, key=lambda x: x['title'])

    st.subheader("Table of Contents - Circulars")
    st.write("Total Circulars:", len(sorted_circulars))
    
    if sorted_circulars:
        data = []
        for index, circular_info in enumerate(sorted_circulars, start=1):
            data.append({
                'S.No.': index,
                'Title': circular_info['title'],
                'File No.': circular_info['file_no'],
                'Issuer': circular_info['issuer'],
                'Date': circular_info['date'],
                'Signatory': circular_info['signatory'],
                'PDF': circular_info['pdf']
            })

        df = pd.DataFrame(data)

        st.write("### Table Columns:")
        st.write("- **S.No.**: Serial number of the circular.")
        st.write("- **Title**: The title of the circular.")
        st.write("- **File No.**: The file number of the circular.")
        st.write("- **Issuer**: The issuing authority of the circular.")
        ...
        for index, row in df.iterrows():
            col1, col2, col3, col4, col5, col6, col7, col8 = st.columns([1, 4, 2, 2, 2, 2, 2, 1])
            col1.write(row['S.No.'])
            col2.write(row['Title'])
            col3.write(row['File No.'])
            col4.write(row['Issuer'])
            col5.write(row['Date'])
            col6.write(row['Signatory'])
            col7.write(row['PDF'])
            if col8.button("✓", key=f"select_circular_{index}"):
                st.session_state.selected_pdf = row['PDF']
                st.session_state.current_option = 'Show Original Document'
                st.experimental_rerun()
...
def main():    
    ...
    if st.session_state.authenticated:
        ...
        option = st.sidebar.radio("Choose an option:", ('Show Original Document', 'Show Extracted Document', 'View Sections', 'Dictionary Lookup', 'User Manual', 'Bare Acts - Table of Contents', 'Update Circulars', 'Circulars - Table of Contents'))
        ...
        if st.session_state.selected_pdf and st.session_state.current_option == 'Show Original Document':
            if option == 'Circulars - Table of Contents':
                pdf_folder = r'D:\Documents\Lex Res\Lex Res BARE ACTS Engine\Circulars_44'
            else:
                pdf_folder = 'All Documents'
        ...
        elif option == 'Circulars - Table of Contents':
            circular_folder = r'D:\Documents\Lex Res\Lex Res BARE ACTS Engine\Circulars_44'
            metadata_folder = r'D:\Documents\Lex Res\Lex Res BARE ACTS Engine\Cleaned Metadata - Circular'
            titles_folder = r'D:\Documents\Lex Res\Lex Res BARE ACTS Engine\Cleaned Metadata - Circular - Titles'
            circulars = extract_circular_metadata(circular_folder, metadata_folder, titles_folder)
            
            if circulars:
                sort_option = st.sidebar.selectbox("Sort by:", ('Title', 'File No.', 'Issuer', 'Date', 'Signatory'))
                display_circulars_table_of_contents(circulars, sort_option)
            else:
                st.write("No circulars found.")
